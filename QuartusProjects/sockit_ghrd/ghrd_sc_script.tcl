...
proc led_on {pattern} {
  global mj_nsec
  global pio_led_base
  set dvalue [expr $pattern]
  regwr [ expr $pio_led_base ] $dvalue
}
...
proc hps_led_on {pattern} {
  ...
  secwr [ expr $hps_led_base + 4] 0x0f000000
  #secwr [ expr $hps_led_base ] [ expr $pattern<<12 ]
  secwr [ expr $hps_led_base ] [ expr $pattern << 24 ]
}
...
proc hps_led_off {} {
  global mj_secu
  global hps_led_base
  secwr [ expr $hps_led_base ] 0x00000000
}
...
proc hps_led_run {interval occurance} {
  ...
  for {set y 0} {$y<$occurance} {incr y} {
    ...
    for {set x 0} {$x<4} {incr x} {
      set dvalue [expr ~$value]
      secwr $hps_led_base [expr $dvalue << 24 ]
